...
class UXAuditor:
    ...
    def audit_file(self, filepath: str) -> None:
        # Robust filtering for node_modules and hidden folders
        if any(x in filepath for x in ['node_modules', '.next', 'dist', '.agent', '.claude', '.qoder', '.git', '.vscode']):
            return
        ...
        # --- FILTER: MACRO vs MICRO CHECKS ---
        is_page = any(x in filename.lower() for x in ['page', 'index', 'layout', 'home', 'app']) or filepath.endswith('.html')
        is_component = not is_page

        # --- 1. PSYCHOLOGY LAWS (Page Level) ---
        if is_page:
            # Hick's Law
            ...
            # ... (Fitts Law is micro, keep it outside?)
            # Fitts' Law actually applies to buttons in components too.
        ...
        # Miller's Law (Page Level - Complex Forms)
        if is_page:
        ...
        # Reflective: Brand story (Page Level)
        if is_page:
