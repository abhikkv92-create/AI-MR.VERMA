...
1. **Analyze Request**
   - Understand the user's specific need regarding **verification-before-completion**.
   - Identify if the task involves verification-before-completion best practices, specific patterns, or tool usage.

2. **Activate Skill**
   - Use the `view_file` tool to read the full instructions in:
     `E:\ABHINAV\AI KIT\.agent\skills\verification-before-completion\SKILL.md`
