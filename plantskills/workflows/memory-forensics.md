...
1. **Analyze Request**
   - Understand the user's specific need regarding **memory-forensics**.
   - Identify if the task involves memory-forensics best practices, specific patterns, or tool usage.

2. **Activate Skill**
   - Use the `view_file` tool to read the full instructions in:
     `E:\ABHINAV\AI KIT\.agent\skills\memory-forensics\SKILL.md`
