...
1. **Analyze Request**
   - Understand the user's specific need regarding **free-tool-strategy**.
   - Identify if the task involves free-tool-strategy best practices, specific patterns, or tool usage.

2. **Activate Skill**
   - Use the `view_file` tool to read the full instructions in:
     `E:\ABHINAV\AI KIT\.agent\skills\free-tool-strategy\SKILL.md`
