...
## 🕸️ Spider Web Sync
- **Integrated Optimizations**: Apply `@[/poweruseage]` Level 3 + `@[/memory-optimization]` + `@[/using-superpowers]`.
- **System Hub**: This workflow is the central hub for all 28 specialist agents and 126 skills.
- **Related Triggers**: `/status`, `/audit`, `/blueprint`, `/plan`, `/synthesize`.
...
- **Related Triggers**: `/frontend-specialist`, `/backend-specialist`, `/database-architect`, `/security-auditor`, `/test-engineer`, `/qa-automation-engineer`, `/deploy`, `/performance-optimizer`, `/mobile-developer`, `/documentation-writer`, `/plan`, `/debug`.
