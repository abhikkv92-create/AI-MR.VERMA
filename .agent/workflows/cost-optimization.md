...
1. **Analyze Request**
   - Understand the user's specific need regarding **cost-optimization**.
   - Identify if the task involves cost-optimization best practices, specific patterns, or tool usage.

2. **Activate Skill**
   - Use the `view_file` tool to read the full instructions in:
     `E:\ABHINAV\AI KIT\.agent\skills\cost-optimization\SKILL.md`
