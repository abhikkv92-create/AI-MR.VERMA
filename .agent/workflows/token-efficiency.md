---
description: Token optimization best practices for cost-effective Claude Code usage. Automatically applies efficient file reading, command execution, and output handling strategies. Includes model selection guidance (Opus for learning, Sonnet for development/debugging). Prefers bash commands over reading files. version: 1.4.0
---

# token-efficiency Workflow

1. **Analyze Request**
   - Understand the user's specific need regarding **token-efficiency**.
   - Identify if the task involves token-efficiency best practices, specific patterns, or tool usage.

2. **Activate Skill**
   - Use the `view_file` tool to read the full instructions in:
     `E:\ABHINAV\AI KIT\.agent\skills\token-efficiency\SKILL.md`
